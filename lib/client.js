...
//      以及「应用」按钮——外观改动点应用后才提交，避免即时应用不可靠。
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// ------------------------------------------------------------------
		// 设置页「任务小窗」区块：显示/隐藏开关 + 强调色 + 透明度 + 应用按钮
		// 外观改动（颜色/透明度）仅本地暂存，点击「应用」才提交到宿主。
		// ------------------------------------------------------------------
		function TaskWidgetSettingsSection() {
			...
			const [base, setBase] = (0, react.useState)({ accent: "#4D9FFF", alpha: 100, cardAlpha: 100 });
				...
				fetch("/dsh-task-widget/api/ctl?action=status")
					...
					.then((j) => {
						...
						if (j.cfg) {
							...
							setBase({ accent: j.cfg.accent, alpha: j.cfg.alpha, cardAlpha: j.cfg.cardAlpha || 100 });
						}
						setHint(running
							? (j.visible === true ? "桌面小窗当前显示中（右上角）" : "桌面小窗已隐藏，任务完成时自动弹回")
							: "桌面小窗未运行，打开后将自动启动");
					})
			...
			const onApply = () => {
				if (applying) return;
				setApplying(true);
				fetch("/dsh-task-widget/api/ctl?action=config&accent=" + encodeURIComponent(accent) + "&alpha=" + alpha + "&cardAlpha=" + cardAlpha, { method: "POST" })
					.then((r) => r.ok ? r.json() : null)
					.then((j) => {
						if (j && j.ok) {
							setBase({ accent, alpha, cardAlpha });
							setApplied(true);
