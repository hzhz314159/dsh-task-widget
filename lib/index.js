...
//      Win11/Win10 液态玻璃、始终置顶、可拖动、深/浅色跟随系统、
  ...
  return ctx.effect(() => {
    ...
    function collect() {
      ...
      for (const [id, prev] of state.running) {
        if (!liveIds.has(id) && prev && !state.recentDone.some((r) => r.id === id)) {
          ...
          state.recentDone.unshift({ id, title: known ? known.title : id, doneAt: Date.now() });
      ...
      const snap = {
        ts: Date.now(),
        app: { version: '0.3.1', widgetUrl: widgetBaseUrl() },
    ...
    function onAgentStatus({ agent, status }) {
      ...
      if (prev === true && !running) {
        ...
        state.recentDone.unshift({ id, title, doneAt: Date.now() });
        if (state.recentDone.length > MAX_RECENT_DONE) state.recentDone.length = MAX_RECENT_DONE;
        pushEvent('session-done', '会话完成', title);
        // 用户关过小窗 → 任务完成时自动弹回
      ...
      if (projections && typeof projections.snapshot === 'function') {
        ...
        if (sig && sig !== prev) {
          ...
          if (allDone && !prevAllDone && todos.length > 0) {
            ...
            pushEvent('todos-done', '任务全部完成', title + ' · ' + todos.length + ' 项');
          }
        }
      }
      markDirty();
    ...
    function onJobDone(snapshot) {
      ...
      pushEvent('job-done', '后台任务完成', (title ? title + ' · ' : '') + String(v.label || v.id || ''));
      markDirty();
    }
    ...
    function handleRoute(req, res) {
      ...
      try {
        ...
        if (p === '/dsh-task-widget/api/ctl') {
          ...
          if (action === 'config') {
            // 软件内调整：强调色 + 透明度 → 写 .config.json 并通知小窗热应用
            ...
            const lockedRaw = u.searchParams.get('locked');
            const cfg = { accent: '#4D9FFF', alpha: 100, cardAlpha: 100, locked: false };
            try {
            ...
            if (lockedRaw !== null) cfg.locked = lockedRaw === '1' || lockedRaw === 'true';
            try {
          ...
          if (action === 'status') {
            return json(res, 200, {
              ...
              pendingActivate: state.pendingActivate,
              cfg: safe(() => { const c = JSON.parse(readFileSync(CONFIG_FILE, 'utf8')); return { accent: String(c.accent || '#4D9FFF'), alpha: Number(c.alpha) || 100, cardAlpha: Number(c.cardAlpha) || 100, locked: Boolean(c.locked) }; }, { accent: '#4D9FFF', alpha: 100, cardAlpha: 100, locked: false }),
